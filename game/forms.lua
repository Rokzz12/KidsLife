...
function build_form(index)
  if transition_done == true then
    ...
    if index == 3 then --main menu
      ...
      add_button(7, s_age .. "...", 1350, 850, 400)
    ...
    elseif index == 12 or (index >= 43 and index <= 53) then --game word builders
      ...
      try_counter = 1
        ...
        table.sort(a_names_sorted, function(a,b) return #a.name<#b.name end)
          ...
          while(animal_name == "" or animal_name == nil or random_n == previous_random_n) do
            local upper_limit = math.ceil((table_length(a_names_sorted) / 5) * (selected_age - 1))
            local lower_limit = math.ceil((table_length(a_names_sorted) / 5) * (selected_age))
        ...
        correct_row = tiles[7]
        random_n = selected_age + 1--love.math.random(2, utf8.len(animal_name))
      ...
      elseif index == 13 or index == 33 or (index >= 54 and index <= 65) then -- number spelling game   or   shape matching   or   word matchers games
        ...
        if index >= 54 and index <= 65 then
          init_images(group_name)
        end
        if selected_age == 1 or selected_age == 2 then
          game_table_rows = 3
          game_table_y_offset = 0
        elseif selected_age == 3 or selected_age == 4 then
          game_table_rows = 4
      ...
      elseif index == 15 then -- Shopping List
        ...
        for i = 1, selected_age + 1 do------ difficulty
          items_needed[i] = {}
      ...
      elseif index == 18 then
        add_big_button(400, icon_back, screen_left + 80, screen_top + 100, 55, 0.38)
        add_button(1, s_age .. " 5", 800, 300, 370)
        add_button(2, s_age .. " 7", 800, 400, 370)
        add_button(3, s_age .. " 10", 800, 500, 370)
        add_button(4, s_age .. " 12", 800, 600, 370)
        add_button(5, s_age .. " 14", 800, 700, 370)
        fade_from_white(0.03, true)
      elseif index == 19 then -- main menu (new)
        initialize_decoration_elements()
        if username == "admin" then
          add_button(3, s_age .. "...", 350, 660, 485)
          add_button(4, s_change_language, 350, 745, 485)
          add_button(5, s_copyright, 350, 830, 485)
          add_button(6, s_manage_users, 350, 575, 485)
        else
          add_button(3, s_age .. "...", 350, 660 - 40, 485)
          add_button(4, s_change_language, 350, 745 - 40, 485)
          add_button(5, s_copyright, 350, 830 - 40, 485)
        end

        if game == "math" then
          add_big_button(8, icon_m1,          760, 163, 57, 0.5, false, 0, 0, 20, s_numbers, true)
          ...
          --for user system
          --add_big_button(8, icon_m1,          792, 283, 57, 0.5, false, 0, 0, 20,  s_numbers, true)
          --add_big_button(9, icon_m4,          824, 403, 57, 0.5, false, 0, 0, 21, s_patterns, true)
          --add_big_button(10,icon_ico_g_0303,  856, 523, 57, 0.5, false, 0, 0, 22,  s_basic_operations, true)
          --add_big_button(11,icon_m2,          888, 643, 57, 0.5, false, 0, 0, 23, s_shapes_and_solids, true)
          --add_big_button(12,icon_m5,          920, 763, 57, 0.5, false, 0, 0, 24, s_time, true)
      ...
      elseif index == 37 then --time matching
        ...
        for i = 1, 6 do
          ...
          if utf8len(time_words[i]) > 46 then
            time_words[i] = string_in_two_lines(time_words[i])
          end
        end
        for i = 1, 6 do
      ...
      elseif index == 42 then -- trace letters and numbers
        ...
        selected_color = 1
      elseif index == 5000 then
        love.event.quit()
      end--------------------------------------------------------

      -- for user system (if status bar is shown)
      -- if index >= 3 then
