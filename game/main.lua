...
function love.load()
  ...
  --score[user name][age][game index]
  initialize_score()
...
function love.update(dt)
  ...
  if love.mouse.isDown(1) and mouse_released then --mouse click
    ...
    elseif current_window == 18 then -- age select
      for i = 1, 5 do
...
function love.draw()
  ...
  elseif current_window == 5 then 
    ...
    for k = 1, table_length(s_credits) do
      if k == 18 then x = 800 y = 300 end
      if k == 13 then x = 400 y = 170 end
      if k == 14 then x = 400 y = 200 end
      if k == 15 then x = 400 y = 230 end
      if k == 1 or k == 18 or k==26 or k==13 then
        set_font("interface_bold")
      else
        set_font("interface")
      end
  ...
  elseif current_window == 18 then -- age select
    draw_header("Level" .. "- " .. selected_age)
...
--
  ...
  elseif current_window == 26 then --user scores
    ...
    set_font("interface_bold")
    print_text(s_age, column + 1600 / 3, scroll + 160, 1600 / 6, 'center')
    for i = 0, 4 do
  ...
  elseif current_window == 35 or current_window == 36 then -- how clock works, learn to set the clock
    ...
    draw_analog_clock(400, 525, 290, clock_hour, clock_min, 'arabic_digits', selected_age)
    set_font("button_text")
    love.graphics.setColor(color["interface_text"])
    if current_window == 35 then
  ...
  elseif current_window == 37 then -- time matching
    ...
    for i = 1, 6 do
      ...
      draw_analog_clock(196 + (i - 1) * 288 + x, 376 + y, 100, time_clocks[i].hour, time_clocks[i].min, 'arabic_digits_small', selected_age)
    end
  ...
  if current_window >= 3 then
    ...
    --set_font("interface")
    --print_text(s_age .. ": " .. selected_age, screen_left, screen_top - 7, screen_total_width, 'center')
