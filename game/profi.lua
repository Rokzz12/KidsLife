...
--[[
	...
	provide a ages parameter to traceback a number of ages.
	Params: [methodName:string] the name of the method you wish to inspect.
	        [ages:number:optional] the amount of ages you wish to traceback, defaults to 1.
]]
function ProFi:setInspect( methodName, ages )
	if self.inspect then
		self.inspect.methodName = methodName
		self.inspect.ages = ages or 1
	else
		self.inspect = {
...
function ProFi:doInspection( inspect, funcReport )
	...
	local ages = 5 + inspect.ages
	local currentage = 5
	while currentage < ages do
		local funcInfo = debug.getinfo( currentage, 'nS' )
		if funcInfo then
